...
if (ctype_xdigit(str_replace('#', '', $color))) { //Is hex?
  $hex = $color;
  //echo "Hex color: " .$color.'<br />';
}
elseif ((strpos($color, ',') !== false) || (strpos($color, 'rbg') !== false)) {
  $hex = rgb2hex($color);
  //echo "RGB color: " . $color .'<br />Hex conversion: ' . $hex . '<br />';
}
else {
  //echo "Neither hex nor RGB. - " . $color . "|" . str_replace('#', '', $color) . '<br />xdigit: ' . ctype_xdigit(str_replace('#', '', $color));
}
...
$im_handle = imagecreatefrompng($path . $image . '.png');
if (!$im_handle) {
  exit;
}
